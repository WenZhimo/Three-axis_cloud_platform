...
这会影响调试读数。`rollDiag.stepLimit` 当前写入的是 `eepromConfig.rateLimit * safeDt`，不是抬升后的 `actualRollStepLimit`；`rollDiag.dPidRaw` 记录的是限速前差值，`rollDiag.pidApplied` 才是速率限制后参与电角合成的结果。也就是说，当 `rateLimit * safeDt < AXIS_MIN_STEP_LIMIT_RAD` 时，不能只看 `rollDiag.stepLimit` 就断言最终限制阈值等于该字段。
