...
| 调试环节 | 本章对应做法 |
|---|---|
| 现象 | D 项尖峰、目标突变、电机抽动、输出饱和、积分残留、三轴响应不一致。 |
| 可能原因 | D 项未平滑、目标斜坡未接入、限幅/限速位置误读、`iHold` 未触发、脚本与固件不等价。 |
| 定位方法 | 先按下面七步观察 D 项、目标斜坡、输出限幅、速率限制和积分暂停。 |
| 验证步骤 | 分别记录限制前值、限制后值、历史状态和对应源码路径；缺少实测时保留【待验证】。 |
| 解决方案 | 先修正证据层级和调试记录，再决定是否调 PID 参数、限幅、速率限制或门控逻辑。 |
| 经验总结 | 不用单个变量名直接判断控制效果，必须说明变量处在 PID 原始输出、轴级约束还是硬件输出哪一层。 |
