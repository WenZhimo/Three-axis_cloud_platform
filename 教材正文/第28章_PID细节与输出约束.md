...
| 边界类型 | 需要保留的判断边界 |
|---|---|
| 参数与单位 | Pitch/Yaw 的 `rateLimit` 当前未乘以 `dt`，与 Roll 分支不一致，需在调参或后续代码整理中继续验证；`config.c` 注释说明 `rateLimit` 面向 electrical rotation，教材不能把它简化为机械角速度限制；Pitch 轴“额外更严格限幅”代码当前上下限仍与全局 `[-10, 10]` 相同。 |
| 三轴接线差异 | 目标角斜坡机制当前不能写成三轴都已接入；Pitch/Yaw 的本轴积分阈值宏当前不能写成已经生效的本地门控；预留斜坡、Yaw 平滑和 Roll 收敛门控变量需要运行路径证据才能写成已生效机制。 |
| 变量语义与调试 | Roll 的 `rollDiag.stepLimit` 不一定等于实际生效的最终步长限制；`pidCmdPrev[]` 更新发生在速率限制之后，它是下一帧历史基准，不是原始 PID 输出缓存；`outputRate[]` 不能直接当作最终实际输出变化量；原始 D 项 `[-300, 300]` 限幅不能直接等同于每帧 D 输出贡献上限。 |
本章待验证分类：

| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| D项与历史状态 | `updatePID()` 的 D_ERROR/D_STATE 分支、低通、三点平均、限幅和哨兵初始化路径已经进入源码与构建证据。 | 某次运行真实 D 项来源、第一帧历史来源、跨 `±π` 边界响应和目标阶跃冲击。 |
| 三轴输出约束 | Roll、Pitch、Yaw 的幅值限幅、速率限制、`pidCmdPrev[]` 更新和 `outputRate[]` 路径已分轴列清。 | Pitch/Yaw `rateLimit` 单位设计、机械等效速率、真实平滑效果和安全边界。 |
| 积分与门控 | `iTerm` 硬限幅、`iHold` 入参、`holdIntegrators` 同帧传播和 `windupGuard` 未直接参与限幅的源码边界已明确。 | 真实过冲是否由积分残留导致、跨轴暂停是否改善收敛、后续非零 I 参数的影响。 |
