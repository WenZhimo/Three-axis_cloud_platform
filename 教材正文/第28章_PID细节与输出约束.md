...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 参数与单位 | Roll、Pitch、Yaw 的 `rateLimit` 表达式、幅值限幅和 `pidCmdPrev[]` 更新顺序已经分轴列清。 | Pitch/Yaw `rateLimit` 单位设计、机械等效速率、真实平滑效果和安全边界。 |
| 三轴接线与门控 | Roll 目标斜坡、Pitch/Yaw 预留变量、本轴积分阈值宏和全局 `holdIntegrators` 传播路径已按源码区分。 | Pitch/Yaw 目标斜坡是否接入、本轴积分门控是否生效、跨轴暂停是否改善收敛。 |
| 变量语义与调试 | `rollDiag`、`pidCmd[]`、`outputRate[]` 和 `pidCmdPrev[]` 的生命周期已拆分。 | 某次运行中每个阶段的真实值、最终帧间变化量、D 项历史来源和目标阶跃冲击。 |
| 仿真等价性 | `tools/pid_tuning_sim.py` 与固件在部分限幅、滤波和趋势分析上可对照。 | 逐帧数值等价、硬件闭环稳定、温升、电流和机械安全。 |
| 构建与运行一致性 | `.map/.list/.su/.cyclo` 能证明 PID 细节、输出约束和电机调用路径进入某次 Debug 构建。 | 板上烧录镜像、真实 500Hz 耗时、栈水位、示波器波形和硬件响应。 |
