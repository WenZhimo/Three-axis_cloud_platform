...
> 导航：上一章：[第27章_PID控制核心](第27章_PID控制核心.md) ｜ 下一章：[第29章_机械角电角与正弦输出](第29章_机械角电角与正弦输出.md)
...
| 阅读层次 | 建议范围 | 适合读者 |
|---|---|---|
| 【必须掌握】 | 第1节到第5节，第13节总结 | 需要理解 PID 保护主线的读者 |
| 【工程深化】 | 第6节到第8.9节，第9节调试方法 | 需要维护固件、调试三轴差异和定位异常的读者 |
| 【拓展阅读】 | 第8.10节仿真脚本边界，第8.11节预留变量边界，第12节思考题 | 需要使用仿真工具或做后续优化的读者 |
| 【证据与验证】 | 第5.1.1节、第5.5.1节、第8节、第9节、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查 D项滤波与异常保护、目标角斜坡、三轴输出约束、`rateLimit` 单位语义、积分限幅、约束顺序、仿真边界、`.map/.list/.su/.cyclo` 证据或准备硬件验证的读者 |

如果只是沿主线学习，可以先跳过多个“边界”小节，只记住保护顺序和三轴差异；调试或审校时再回到对应证据小节查证。

本章速查：

| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| D 项滤波与异常保护 | 第4.1节到第4.2节、第5.1节到第5.2节、第8.1节到第8.3.1节 | 一阶低通滤波原理回到第22章，PID主公式回到第27章 |
| 目标角斜坡与目标突变 | 第4.3节、第4.6节到第4.7节、第5.3节到第5.4节、第8.4节 | 角度归一化回到第23章 |
| 输出限幅、速率限制和积分边界 | 第4.4节到第4.9节、第5.5节到第5.6节、第8.5节到第8.9.3节 | PID核心状态和 `updatePID()` 输入输出回到第27章 |
| 仿真、预留变量和构建证据 | 第8.10节到第8.12.2节、第9节、章节尾部固定检查 | 电角输出链路继续到第29章，三相 PWM 输出继续到第30章 |
...
| 保护层 | 当前源码证据 | 保护对象 | 不能替代什么 |
|---|---|---|---|
| PID内部数值保护 | `pid.c::updatePID()` 中 `deltaT` 回退、`dTerm` 限幅、D 项低通、三点平均、`iTerm` 硬限幅 | PID 公式内部的时间步、微分噪声和积分状态 | 不能证明轴级输出不会突变，也不能证明电机安全 |
| PID输入侧保护 | Roll 路径的 `moveTowardsAnglef(..., ROLL_TARGET_SLEW_RAD_S * safeDt)` | 进入 PID 前的目标角变化速度 | 不能替代 PID 输出限幅；Pitch/Yaw 当前没有同等运行证据 |
| PID输出侧保护 | `pidCmd[]` 幅值限幅、`outputRate[]` 与 `pidCmdPrev[]` 的变化率限制 | PID 补偿量的大小和帧间变化 | 不能改变原始 D 项已经看到的目标阶跃或测量噪声 |
| 积分门控保护 | `iHold`、`holdIntegrators`、`rollHoldIntegrators` 和 `zeroPIDintegralError()` | 积分状态是否继续累积、启动或使能阶段的历史状态 | 不能替代输出饱和反算 anti-windup |
| 电机设角前边界 | `PWM_Motor_SetAngle()` 调用前的电角合成表达式 | 约束后的 PID 补偿如何进入定子电角 | 不能证明三相 PWM、电机方向和机械响应已经正确【待验证】 |

因此，调试时不要只说“PID 有保护”。应先定位异常发生在哪一层：目标输入跳变、PID 内部 D 项尖峰、输出补偿过大、帧间变化过快、积分历史残留，还是电角合成后的硬件响应异常。层级不同，应该观察的变量和下一步处理也不同。
...
#### 8.6.2 Roll最小步长抬升与诊断字段边界

Roll 路径还有一层容易漏掉的细节：源码先计算 `rollStepLimit = eepromConfig.rateLimit * safeDt`，随后若它小于 `AXIS_MIN_STEP_LIMIT_RAD`，就把本帧实际步长限制抬升到 `AXIS_MIN_STEP_LIMIT_RAD`。因此，Roll 的实际限速阈值不是单纯的 `rateLimit * safeDt`，而是：

```text
actualRollStepLimit = max(eepromConfig.rateLimit * safeDt, AXIS_MIN_STEP_LIMIT_RAD)
```

这会影响调试读数，需要分清三个字段：

- `rollDiag.stepLimit`：当前写入的是 `eepromConfig.rateLimit * safeDt`，不是抬升后的 `actualRollStepLimit`。
- `rollDiag.dPidRaw`：记录限速前差值。
- `rollDiag.pidApplied`：记录速率限制后参与电角合成的结果。

也就是说，当 `rateLimit * safeDt < AXIS_MIN_STEP_LIMIT_RAD` 时，不能只看 `rollDiag.stepLimit` 就断言最终限制阈值等于该字段。

`.list` 能证明当前 Debug 构建中保留了这些静态路径：

- `rollStepLimit < AXIS_MIN_STEP_LIMIT_RAD` 的比较。
- `outputRate[ROLL] > rollStepLimit` 的分支。
- `rollDiag.stepLimit = eepromConfig.rateLimit * safeDt` 的写入。

但 `AXIS_MIN_STEP_LIMIT_RAD` 是否改善低速段手感、是否引入额外抖动，需要真实日志或实物测试继续验证【待验证】。

#### 8.6.3 Pitch/Yaw每帧限速调试边界
...
这个时序决定了调试方法。若怀疑输出饱和导致积分残留，应同时记录：

- `iHold`
- `iTerm_before`
- `iTerm_after`
- `pidRaw`
- `pidCmd_after_amplitude_limit`
- `pidCmd_after_rate_limit`

如果 `pidCmd[]` 已经被限幅，而 `iHold == false` 且 `iTerm_after` 仍继续朝同一方向变化，只能说明当前实现允许积分在硬边界内继续累积；不能把轴级输出限幅误判成反算 anti-windup 已经生效。
...
第8.7至8.12节都围绕“PID 原始输出如何逐层变成可下发的电机命令”展开。阅读时建议先用下面的速查表确认变量处在哪一层，再进入对应小节细看，避免把多个“输出”混成同一个量：

| 输出约束层级 | 优先查看小节 |
|---|---|
| 积分状态是否继续累积 | 8.7、8.7.1、8.7.2、8.8 |
| `updatePID()` 返回的 PID 原始补偿量 | 8.9、8.9.1 |
| 幅值限幅后的 `pidCmd[]` | 8.9、8.9.2 |
| 速率限制、`outputRate[]` 与 `pidCmdPrev[]` | 8.9、8.9.2、8.9.3 |
| 仿真脚本与固件逐帧等价边界 | 8.10 |
| 构建产物能证明的输出约束路径 | 8.12 |
...
真正的轴级保护发生在调用者 `computeMotorCommands()` 内部：

- Roll：先把 `rollPidRaw` 经过 `clampf(..., ±ROLL_CMD_LIMIT_RAD)`，再按 `rollStepLimit = eepromConfig.rateLimit * safeDt` 做帧间步长限制。
- Pitch/Yaw：在 `updatePID()` 后分别检查 NaN/Inf，限制到 `±PITCH_CMD_LIMIT_RAD` 或 `±YAW_CMD_LIMIT_RAD`，再用 `eepromConfig.rateLimit` 限制 `outputRate[axis]`。

这个边界对调参很关键。如果只在 `updatePID()` 入口或返回处打点，只能看到 PID 原始补偿量。若要判断“是否被幅值限幅截断”或“是否被速率限制压住”，必须同时记录：

- 限幅后的 `pidCmd[axis]`。
- 限速前的 `outputRate[axis]`。
- 限速后的 `pidCmd[axis]`。
- 更新前的 `pidCmdPrev[axis]`。

`.map` 能证明 `computeMotorCommands`、`updatePID`、`clampf` 进入最终镜像。`.list` 能证明三轴路径中 `updatePID()` 调用、NaN/Inf 回退、幅值限幅和 `outputRate[]` 比较分支存在。

但这些证据不能证明某一次实测中电机电角、PWM波形或机械角响应已经正确，仍需在线日志、断点或示波器观测【待验证】。
...
#### 8.9.3 Roll诊断结构与Pitch/Yaw手动打点边界

`computeMotorCommands.h` 中定义了 `rollDiag_t`，并暴露 `extern rollDiag_t rollDiag`。

这个结构体记录 Roll 的目标角、当前角、机械误差、电角误差、`pidRaw`、`pidClamped`、`pidApplied`、`dPidRaw`、`stepLimit`、符号配置和 `holdI`。因此 Roll 可以通过一个结构体同时观察“输入、误差、PID原始输出、约束后输出和积分暂停”多个阶段。

Pitch/Yaw 当前没有对应的 `pitchDiag_t` 或 `yawDiag_t`。

源码中能直接复用的全局证据主要是 `pidCmd[]`、`outputRate[]` 和 `pidCmdPrev[]`。`pidRaw`、幅值限幅前后值、`iHold` 入参和电角合成中间量，需要在对应分支临时打点或断点观察。

构建产物能证明的是另一层边界：

- `.map` 能证明 `.bss.rollDiag`、`.bss.outputRate`、`.bss.pidCmdPrev` 被分配到 RAM。
- `.list` 能证明 Roll 写入 `rollDiag` 字段，而 Pitch/Yaw 只沿共享数组路径更新。
...
#### 8.10.2 脚本`rate_limit`与固件三轴限速边界
...
`tools/pid_tuning_sim.py::update_pid()` 本身有 `i_hold` 入参：
...
但主循环当前固定调用 `update_pid(..., False, pid)`。这意味着脚本模拟的是“每一帧都允许积分累积”的场景。
固件当前积分门控更细：

| 场景 | 固件证据 | 脚本当前是否模拟 | 教材判读 |
|---|---|---|---|
| Roll 本轴误差过大 | `rollHoldIntegrators = fabsf(roll_error_electrical) > ROLL_I_ENABLE_ERR_RAD`，并作为 Roll 的 `iHold` 传入 `updatePID()` | 否，脚本固定 `i_hold=False` | 脚本可能高估 Roll 积分累积，尤其在大误差收敛阶段 |
| 同帧跨轴传播 | Roll 触发暂停时把全局 `holdIntegrators` 置真，后续 Pitch/Yaw 调用读取该全局值 | 否，脚本三轴各自独立循环且固定允许积分 | 脚本不能证明 Pitch/Yaw 在固件中一定持续积分 |
| 启动或重新使能清零 | `main()` 启动门控调用 `zeroPIDintegralError()`，Roll 首次使能时清 `iTerm` | 否，脚本没有复现这些状态机时序 | 脚本曲线不能替代固件启动瞬间的积分状态记录 |
| 硬限幅 | `updatePID()` 与脚本都把积分状态限制到 `[-10, 10]` | 是，思想一致 | 只能说明硬限幅形态相近，不能证明门控时序等价 |

所以，若脚本用于估计 I 参数趋势，应把结论限定为“持续允许积分时的趋势”。
若要验证固件积分暂停，应回到断点或日志记录 `rollHoldIntegrators`、全局 `holdIntegrators`、
三轴 `iHold` 入参、`iTerm_before/iTerm_after` 和 `zeroPIDintegralError()` 是否刚执行【待验证】。
`.list` 能证明当前 Debug 构建中这些门控和清零路径存在，但不能证明某次实测的积分状态已经按预期演化。

但脚本使用的是简化二阶对象模型，不等于真实电机、真实 IMU 和真实 PWM 输出。它适合做趋势理解和调参预演，不能替代仓库外实测验证。

#### 8.10.4 仿真对象模型与硬件闭环边界
...
换成控制系统语言看，脚本假定被控对象是一个受阻尼项和弹性回复项约束的二阶角度系统，PID 输出 `u` 经过 `plant_gain` 直接变成角加速度输入。

这个模型适合解释 P/I/D 参数、D 项滤波、幅值限幅和速率限制对曲线趋势的影响：

- `wn` 越高，模型自然响应越快。
- `zeta` 越低，曲线越容易出现振荡。
- `plant_gain` 越大，同样的 PID 输出会产生更大的角加速度。

但这个模型没有覆盖当前固件真正下发到电机前的执行链。源码路径是 `computeMotorCommands()` 先合成各轴定子电角，再调用 `PWM_Motor_SetAngle()`；后者查正弦表计算三相 `ccr_val[]`，并按轴写入 TIM2/TIM3/TIM4 的 `CCR` 寄存器。
...
- 定时器比较寄存器更新
- 电机驱动器、电机电流和负载惯量
- 摩擦、安装刚度、IMU 采样噪声与反馈延迟

脚本没有模拟这些环节。

所以，本章可以把仿真脚本用于“趋势级预演”：观察参数变化是否可能导致更大的超调、慢收敛、D 项尖峰或限速压制；不能把仿真曲线写成“真实电机闭环已经稳定”。

构建产物只能证明静态路径：

- `Debug/Three-axis_cloud_platformV2.map` 能证明 `PWM_Motor_SetAngle` 进入最终镜像。
- `Debug/Three-axis_cloud_platformV2.list` 能证明三轴分支存在 `updatePID()`、限幅、限速和 `PWM_Motor_SetAngle()` 调用路径。

但这些构建产物仍不能证明真实 PWM 波形、相电流、温升、机械响应和 IMU 反馈闭环已经正确。上述硬件闭环结论必须保留为【待验证】。
...
但权威资料层不能替代项目源码层。本章按三层边界书写：

| 层级 | 可以说明 | 不能替代 |
|---|---|---|
| 资料层 | anti-windup 常见形式包括条件积分、积分暂停、积分限幅、tracking/back-calculation 等；D 项滤波和设定值权重常用于降低噪声敏感性与设定值冲击。 | 不能证明本项目已经实现完整反算 anti-windup，也不能证明当前三轴稳定性和硬件安全。 |
| 源码层 | 当前 `updatePID()` 实现了 `iHold` 条件积分、`iTerm` 硬限幅到 `[-10, 10]`、`dErrorCalc` 分支、一阶低通、三点平均、D 项原始限幅，以及 ANGULAR/非 ANGULAR 两类输出表达式。 | 当前未看到把输出饱和误差反馈回积分状态的反算路径，因此只能写成“积分暂停 + 积分硬限幅”。 |
| 待验证层 | 当前默认配置、三轴接线和实际响应需要回到 `config.c`、`pid.c`、`computeMotorCommands.c`、`.map/.list` 与调试日志验证。 | 缺少现场数据时，稳定性和硬件安全结论保持【待验证】。 |
...
#### 8.12.1 `moveTowardsf()` 的静态报告边界

`moveTowardsf()` 当前在 `computeMotorCommands.c` 中有定义，`.su/.cyclo` 也能给出它的静态栈用量和圈复杂度；
但 `.map` 里它只表现为输入段 `.text.moveTowardsf`，而不是像 `clampf()`、
`moveTowardsAnglef()` 那样出现在最终 Flash 地址段，`.list` 里也没有找到它的最终地址或 `bl`
调用点。换句话说，当前 Debug 镜像只能证明这个 helper 被编译过，不能证明它已经成为实际运行路径的一部分。

因此，本章只能把 `moveTowardsf()` 讲成“静态可见的浮点目标逼近 helper”；
不能直接把它写成当前控制链路已经启用的目标斜坡实现。若后续要把它纳入主线讲解，
仍应先补到 `.list` 的调用证据，再讨论它在运行时中的角色【待验证】。

#### 8.12.2 软件约束到硬件输出的证据边界

第5.5.1节已经说明：本章的完整讲解位置是 `pidCmd[]` 这一层的数值约束，不能把它直接等同于第29章和第30章的电角、三相 PWM 与硬件诊断结论。这里不再重复展开控制含义，只补充构建产物能证明的链路边界：
...
由此可见，`.map/.list` 适合作为“路径进入当前 Debug 构建”的证据；运行时触发、硬件安全、闭环稳定、Pitch/Yaw 单位语义、积分门控触发时序和仿真脚本逐帧等价性仍保持【待验证】。

## 9. 调试方法

本节按统一调试结构组织：现象 -> 可能原因 -> 定位方法 -> 验证步骤 -> 解决方案 -> 经验总结。对第28章而言，核心是把 D 项尖峰、目标突变、电机抽动、输出饱和、积分残留和三轴响应不一致，分别定位到 PID 原始输出、目标斜坡、轴级限幅、速率限制、积分暂停或后续电机输出层。

### 9.1 现象与可能原因

常见现象包括 D 项尖峰、目标突变、电机抽动、输出饱和、积分残留和三轴响应不一致。可能原因包括 D 项未平滑、目标斜坡未接入、限幅/限速位置误读、`iHold` 未触发、脚本与固件不等价，或把第28章的数值约束误当成第29章/第30章的真实硬件输出证据。
...
第28章的调试结论应先固定证据层级，再调整参数。单个变量名不能直接证明控制效果，必须说明它处在 PID 原始输出、轴级约束、电角合成还是硬件输出哪一层；脚本趋势、构建产物和断点记录也不能替代真实电机响应、温升和安全夹具记录。
...
| 问题类型 | 对应问题 | 查阅目的 |
|---|---|---|
| 概念误解 | 问题一、四、六、八、九、十二、十四、十七 | 区分 D 项滤波、积分限幅、目标斜坡、输出限幅和 D 项来源的概念边界 |
| 三轴差异 | 问题二、三、十一、十三、十六 | 查 Roll、Pitch、Yaw 在目标斜坡、`rateLimit`、积分门控和诊断字段上的差异 |
| 调试证据 | 问题七、十、十五 | 区分软件数值约束、变量语义和最终硬件安全之间的证据层级 |
| 仿真边界 | 问题五、十八 | 区分 `tools/pid_tuning_sim.py` 的趋势提示和固件/实物验证 |
...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | `.map/.list/.su/.cyclo` 能证明 PID 细节、输出约束和电机输出调用路径进入某次 Debug 构建。 | 板上烧录镜像、真实 500Hz 耗时、栈水位、示波器波形和硬件响应。 |
| 软件验证 | Roll 目标斜坡、Pitch/Yaw 预留变量、本轴积分阈值宏、全局 `holdIntegrators` 传播路径，以及 `rollDiag`、`pidCmd[]`、`outputRate[]`、`pidCmdPrev[]` 的生命周期已按源码拆分。 | 某次运行中每个阶段的真实值、最终帧间变化量、D 项历史来源、目标阶跃冲击和实际分支行为。 |
| 参数验证 | Roll、Pitch、Yaw 的 `rateLimit` 表达式、幅值限幅和 `pidCmdPrev[]` 更新顺序已经分轴列清。 | Pitch/Yaw `rateLimit` 单位设计、机械等效速率、真实平滑效果和安全边界。 |
| 硬件验证 | `tools/pid_tuning_sim.py` 与固件在部分限幅、滤波和趋势分析上可对照。 | 逐帧数值等价、硬件闭环稳定、温升、电流、机械安全和电机响应。 |
| 官方资料待确认 | Åström/Hägglund、Åström/Murray 和 MathWorks PID 文档可支撑 D 项噪声、设定值冲击、执行器饱和与 anti-windup 的通用背景。 | 本项目 PID 参数语义、输出限幅设计和安全边界仍需结合源码、构建产物与实测继续确认。 |
| 实验待完成 | 本章已经把需要观测的 PID 细节、输出约束和电机调用路径拆成可检查对象。 | 记录逐帧输出、最终帧间变化量、D 项历史、目标阶跃冲击、闭环响应、波形、电流和温升。 |
