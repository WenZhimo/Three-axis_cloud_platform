...
| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| 调参路线、证据层级和回滚点 | 第4.4节到第4.10.1节、第5.4节、第9.2节到第9.3节 | PID核心和输出约束回到第27章、第28章，硬件诊断回到第33章 |
| 仿真工具结构和固件参数来源 | 第4.1节到第4.3节、第5.1节到第5.3节、第8.1节到第8.7.2节 | `updatePID()` 细节回到第27章，D项和限速细节回到第28章 |
| 仿真与固件差异、限幅和状态重置 | 第5.5节到第5.13节、第8.8节到第8.16节 | 电角和三相 PWM 输出链路回到第29章、第30章 |
| 构建产物、仓库外实测和调试记录 | 第8.17节、第9节、章节尾部固定检查 | 真实方向、温升、稳定性和长期运行效果继续保留为仓库外验证 |
...
第8.1至8.17节把 PID 调参与系统优化拆成脚本解析、固件参数来源、限幅/限速、仿真对象、状态清零、指标记录和构建产物证据。阅读时可先用下面的速查表区分“源码文本、仿真模型、目标板实测、构建产物”四类证据，再进入对应小节细看：

| 调参与优化边界 | 优先查看小节 |
|---|---|
| 仿真脚本解析固件默认值、命令限幅和 `rateLimit` | 8.1 至 8.3 |
| 离线 `update_pid()`、`run_simulation()` 和 GUI 能力边界 | 8.4 至 8.6 |
| 固件当前 PID 来源、参数新鲜度和板上 RAM 观测边界 | 8.7 至 8.7.2 |
| `windupGuard`、`holdIntegrators`、速率限制和 D 项差异 | 8.8 至 8.11 |
| 性能证据、仿真对象参数和限幅观察 | 8.12 至 8.14 |
| PID 状态清零、指标记录能力和构建产物证据 | 8.15 至 8.17 |
...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | `.map/.list/.su/.cyclo` 能证明 PID、限速、状态清零、PWM 输出路径和静态资源边界进入某次 Debug 构建。 | 重新构建后的产物一致性、烧录记录、板上运行版本、真实最坏耗时和栈水位。 |
| 软件验证 | 脚本对象模型、D 项差异、限幅/限速路径、指标记录能力、`.su/.cyclo`、`dt500Hz` 和 `executionTime500Hz` 的用途已经区分。 | 板上 RAM 参数值、逐帧数值等价、DWT/GPIO 计时和长期实时裕量仍需运行记录。 |
| 参数验证 | 默认值、当前源码覆盖值、仿真解析值、`.map/.list` 构建值和板上 RAM 值的层级已经分开。 | 目标板实际参数值、方向链路、P/I/D 调整幅度、停止条件和回滚点仍需按单变量调参路线确认。 |
| 硬件验证 | 本章已把调参安全效果与硬件前置检查分开，明确 PID 效果还受门控、限幅、速率限制、电角换算和硬件映射影响。 | 方向正确、真实惯量、摩擦、噪声、供电、结构共振、不过热、无撞限、抗扰稳定、温升、电流和长期闭环安全。 |
| 官方资料待确认 | 经典 PID 理论、离线仿真、源码/构建产物、板上观测和硬件安全记录已经分成不同证据层级。 | 若继续引用 PID 调参、安全裕量或电机控制资料，需要确认其与本项目门控、限幅、速率限制和硬件映射一致。 |
| 实验待完成 | 本章已经形成“先确认硬件和姿态，再用仿真预演，最后小步仓库外实测”的收束路线。 | 重新构建、烧录记录、板上参数快照、逐帧日志、DWT/GPIO 计时、温升、电流、抗扰稳定和仓库外实测记录。 |
...
以下清单用于追溯调参工具、固件 PID 路径和运行证据来源，建议按“工具脚本 -> 固件源码 -> 构建产物 -> 函数 -> 变量 -> 配置项”的顺序分组查阅，不必线性阅读全部条目。
