...
| 【证据与验证】 | 第8.17节构建产物证据边界、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查源码/构建证据或准备仓库外实测记录的读者 |
...
本节只根据当前仓库说明文件、函数、宏、变量、调用关系和 Debug 构建产物。运行时频率、外部硬件表现、主机侧现象、传感器方向、电机响应、板上镜像版本或真实控制效果仍需调试记录、日志、烧录记录或仓库外实测证据；缺少证据时保持【待验证】。
...
第34章的调参结论应先证明调参对象可信，再证明参数更优。固件默认值、仿真曲线、源码变量、构建产物、板上 RAM、阶跃指标和硬件安全记录属于不同证据层级；没有方向链验证、状态基线、限幅/限速记录、指标口径和回滚点时，不把某组 P/I/D 写成“已优化参数”。
...
- `.map` / `.list` 可以证明 PID、限速、状态清零和 PWM 输出路径进入某个 Debug 构建产物，但不能替代烧录记录和仓库外实测。
- `.su` / `.cyclo` 可以给出调参链路关键函数的静态栈估计和圈复杂度边界，但不能替代板上耗时、栈余量、方向和稳定性验证。
