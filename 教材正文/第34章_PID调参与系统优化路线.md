...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 参数与证据新鲜度 | 默认值、当前源码覆盖值、仿真解析值、`.map/.list` 构建值和板上 RAM 值的层级已经分开。 | 重新构建后的产物一致性、板上烧录镜像和目标板实际参数值。 |
| 仿真与实物差异 | 脚本对象模型、D 项差异、限幅/限速路径和指标记录能力已经按源码说明。 | 真实惯量、摩擦、噪声、供电、结构共振和逐帧数值等价。 |
| 调参安全效果 | 单变量调参、停止条件、回滚点和观测指标口径已经形成路线。 | 方向正确、不过热、无撞限、抗扰稳定和长期闭环安全。 |
| 性能与资源边界 | `.su/.cyclo`、`dt500Hz`、`executionTime500Hz` 的用途已经区分。 | 真实最坏耗时、栈水位、DWT/GPIO 计时和长期实时裕量。 |
| 构建与运行一致性 | `.map/.list/.su/.cyclo` 能证明 PID、限速、状态清零、PWM 输出路径和静态资源边界进入某次 Debug 构建。 | 烧录记录、板上运行版本、硬件响应、温升、电流和仓库外实测记录。 |
