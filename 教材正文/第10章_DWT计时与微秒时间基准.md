...
| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| `micros()` 时间戳、`DWT_Delay_us()` 忙等和用途边界 | 第4节到第5节、第13节 | CMSIS寄存器访问回到第05章，SysTick节拍回到第09章 |
| DWT启用、`CYCCNT`、`SystemCoreClock` 和微秒换算 | 第6节、第8.1节到第8.3节、第9节 | 系统时钟树回到第06章，不把 DWT 忙等写成调度器 |
| 温度补偿、静态零偏采样等待和 TIM6 取舍边界 | 第7节、第8.4节到第8.6节、第10节 | MPU6050标定与缩放继续到第21章，500Hz主循环继续到第26章 |
| 构建产物、`CYCCNT` 读回和外部计时验证边界 | 第8.7节到第8.7.1节、章节尾部固定检查 | `.map/.list` 不能替代 GPIO 翻转、示波器或逻辑分析仪实测 |
...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | 当前 Debug 构建中 `micros()`、`DWT_Delay_us()` 与 `HAL_GetTick()` 都生成了 `.su/.cyclo` 静态栈和圈复杂度条目。 | 这些条目只能说明本次编译的静态估算，不能替代运行时最坏栈深度、500Hz 抖动、采样周期或构建新鲜度实测证据。 |
| 软件验证 | `micros()` 和 `DWT_Delay_us()` 都属于时间工具，但前者服务时间戳读取，后者服务微秒忙等等待；`HAL_Delay()` 属于 HAL 毫秒 tick 路径。 | 不能把三者合并解释为同一条时间路径，也不能把 `HAL_Delay()` 写成 DWT 微秒忙等。 |
| 参数验证 | `DWT_Delay_us()` 当前使用整数除法和 32 位乘法，1000us 调用安全；静态零偏采样使用 `DWT_Delay_us(1000)`，温度补偿采样使用 `DWT_Delay_us(sampleRate)`。 | 长等待、移植时钟、采样总周期、传感器新鲜样本和循环开销仍需重新审查或实测。 |
| 硬件验证 | 本章提供后续观测 `dt500Hz`、执行时间和短间隔无符号差值的基础。 | 不证明 500Hz 周期没有抖动、丢帧或长尾延迟，也不证明 1000us 等待对应精确 1000Hz 采样频率。 |
| 官方资料待确认 | 当前仓库能证明 TIM6 有初始化和清零证据，但 DWT/CYCCNT 才是本章 `micros()` 与 `DWT_Delay_us()` 的微秒路径核心。 | 不能把 TIM6 配置意图等同于当前微秒时基来源；DWT 使能、CYCCNT 语义和移植时钟仍需结合 CMSIS/芯片资料确认。 |
| 实验待完成 | 本章已经把微秒时间戳、微秒忙等、HAL 毫秒等待、TIM6 证据和后续 500Hz 观测入口拆成可检查对象。 | 后续需记录运行日志、断点、外部测量、500Hz 抖动、采样周期、传感器新鲜样本、循环开销和最坏栈水位。 |
