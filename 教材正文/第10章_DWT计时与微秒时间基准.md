...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 时间工具边界 | `micros()` 和 `DWT_Delay_us()` 都属于时间工具，但前者服务时间戳读取，后者服务微秒忙等等待 | 不能把两者合并解释为同一条时间路径，也不能把 `HAL_Delay()` 写成 DWT 微秒忙等 |
| 周期观测边界 | 本章提供后续观测 `dt500Hz`、执行时间和短间隔无符号差值的基础 | 不证明 500Hz 周期没有抖动、丢帧或长尾延迟；真实周期仍需运行日志、断点记录或外部测量 |
| 采样等待边界 | 源码能证明静态零偏采样使用 `DWT_Delay_us(1000)`，温度补偿采样使用 `DWT_Delay_us(sampleRate)` | 不证明 1000us 等待对应精确 1000Hz 采样频率；采样总周期、传感器新鲜样本和循环开销仍需实测【待验证】 |
| TIM6 证据边界 | 当前仓库能证明 TIM6 有初始化和清零证据 | 不能把 TIM6 配置意图等同于当前 `micros()` 或 `DWT_Delay_us()` 的微秒时基来源 |
