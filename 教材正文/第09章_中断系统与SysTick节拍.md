...
| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| 中断入口、NVIC、SysTick 1ms 与 `frame_500Hz` 主线 | 第4节到第5节、第8.1节到第8.7节、第13节 | 系统时钟回到第06章，DWT微秒计时继续到第10章 |
| HAL tick、SysTick重配置和主循环500Hz消费 | 第6节、第7节、第8.2节到第8.8节、第9节 | 500Hz控制循环完整数据流继续到第26章 |
| TIM8、USB外设IRQ和系统异常/外设IRQ边界 | 第8.9节、第10节 | TIM8/TIM6配置回到第13章，USB设备栈回到第15章 |
| 构建产物、pending/active状态和运行验证边界 | 第8.10节到第8.11节、章节尾部固定检查 | `.map/.list` 不能替代 DWT/GPIO计时、断点命中或日志证据 |
...
还要把“调用 HAL 设置优先级”继续拆成更小的可验证单元。`HAL_NVIC_SetPriority(IRQn, PreemptPriority, SubPriority)` 本身不直接写最终寄存器值，而是经过三步软件链路：

- 先读取当前 `SCB->AIRCR[10:8]` 中的优先级分组。
- 再调用 CMSIS 的 `NVIC_EncodePriority()`，把抢占优先级和子优先级编码成一个逻辑优先级值。
- 最后交给 `NVIC_SetPriority()` 写入系统异常或外设 IRQ 的优先级字段。
...
因此，在本项目当前分组下，优先级编码结果应分开看：

- `HAL_NVIC_SetPriority(SysTick_IRQn, 0, 0)` 编码为逻辑优先级 0，写入寄存器字段时为 `0x00`。
- `HAL_NVIC_SetPriority(USB_LP_CAN1_RX0_IRQn, 0, 0)` 编码为逻辑优先级 0，写入寄存器字段时为 `0x00`。
- `HAL_NVIC_SetPriority(TIM8_UP_IRQn, 1, 0)` 编码为逻辑优先级 1，写入寄存器字段时为 `0x10`。
- 这里的 `SubPriority = 0` 不是“项目暂时不用子优先级”的经验判断，而是在 `NVIC_PRIORITYGROUP_4` 下没有子优先级位可供编码。

`Debug/Three-axis_cloud_platformV2.list` 也能从构建结果侧证明这条软件路径进入了当前镜像：

- `HAL_NVIC_SetPriority()` 内部调用 `__NVIC_GetPriorityGrouping()`。
- 随后调用 `NVIC_EncodePriority()`。
- 最后调用 `__NVIC_SetPriority()`。

这类证据能说明“优先级值如何被编码并写入哪个寄存器族”，但不能证明运行时某个 IRQ 已经 pending、active 或完成抢占；这些状态仍需 `SCB->SHP`、`NVIC->IP/ISER/ISPR/IABR` 寄存器读回、断点命中或日志证据【待验证】。
...
还要注意，`TIM8_UP_IRQn` 在 STM32F103 设备头文件中还有 `TIM8_UP_TIM13_IRQn`、`TIM13_IRQn` 别名。

这条中断线需要分三层理解：

- 名称含义：IRQn 名称首先表达的是 NVIC 中断线编号和向量入口，不自动等价于唯一外设业务来源。
- 当前项目证据：当前项目调用的是 `TIM8_UP_IRQn`，处理函数名也是 `TIM8_UP_IRQHandler()`，所以本章只按 TIM8 更新入口记录证据。
- 迁移边界：若在其它工程中启用 TIM13，则同一条 IRQ 线还需要结合外设 pending 标志和 HAL 句柄判断实际来源。
...
`.map/.list` 还能把本节结论再拆成三类：

- 进入镜像的入口：`SysTick_Handler`、`USB_LP_CAN1_RX0_IRQHandler`、`TIM8_UP_IRQHandler` 等中断入口进入当前镜像。
- 进入镜像的 HAL 分发：`HAL_InitTick`、`HAL_IncTick`、`HAL_SYSTICK_Config`、`HAL_NVIC_SetPriority`、`HAL_NVIC_EnableIRQ`、`HAL_PCD_IRQHandler` 和 `HAL_TIM_IRQHandler` 等 HAL 函数进入当前镜像。
- 仍需现场验证的结论：`.su/.cyclo` 不能简单相加为系统最大栈深，也不能换算成 ISR 真实耗时；中断实际频率、抢占延迟、pending/active 状态、`frame_500Hz` 丢触发风险和 USB/TIM8 业务效果仍需断点、寄存器窗口、DWT/GPIO 计时、日志或外部仪器证据【待验证】。
