...
> 导航：上一章：[第07章_MSP初始化与GPIO复用](第07章_MSP初始化与GPIO复用.md) ｜ 下一章：[第09章_中断系统与SysTick节拍](第09章_中断系统与SysTick节拍.md)
...
| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| AFIO 重映射、SWD 保留和 USART3 部分重映射的主线关系 | 第4节到第5节、第13节 | GPIO 复用基础回到第07章，UART 输出继续到第11章 |
| PA13/PA14 串行调试接口与 JTAG/SWD 边界 | 第6节、第8.1节到第8.2节、第8.5节 | 不把关闭 JTAG 写成关闭 SWD 或释放 PA13/PA14 |
| USART3 为什么落到 PC10/PC11 | 第8.3节到第8.4节、第10节第3问和第7问 | 不重复展开 USART 数据收发、printf 重定向或下载流程 |
| `.ioc/Core/Src/.launch` 与构建产物证据边界 | 第8.5节到第9.6节、章节尾部固定检查 | `.map/.list/.su/.cyclo` 不能替代运行时寄存器、GDB 日志或外部串口证据 |
...
本节按“现象 -> 可能原因 -> 定位方法 -> 验证步骤 -> 解决方案 -> 经验总结”组织。第08章调试的目标是确认 AFIO 重映射和调试接口保留没有互相冲突。
...
PA13/PA14 的 SWD 保留链解决调试接口问题，PC10/PC11 的 USART3 partial remap 解决串口引脚问题。两条链都经过 AFIO，但不能把串口无输出直接写成 SWD 配置错误，也不能把 ST-LINK 可连接写成 USART3 已验证。

### 9.6 经验总结：调试边界

本章的证据边界是：`.ioc` 和生成代码可以证明 PA13/PA14 被保留为串行调试接口，USART3 被部分重映射到 PC10/PC11。但 ST-LINK 实际连线、目标板供电、外部串口转接和 PC 端工具配置仍属于仓库外实测证据，缺少实测时保持【待验证】。
