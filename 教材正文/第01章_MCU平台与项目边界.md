...
| 【证据与验证】 | 第8节、第9节、第11节，只把仓库内证据写成已验证事实，实物板状态继续保留【待验证】 | 准备复查平台字段、编译字段、CMSIS设备头、启动文件、链接脚本、Debug构建产物或后续接入下载调试记录的读者 |
...
| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| 目标芯片、封装和平台命名层级 | 第3节到第5.2节、第13节 | 不把 `STM32F103RCT6`、`STM32F103RCTx`、`STM32F103xE` 和 `LQFP64` 混成同一概念 |
| `.ioc/.cproject/CMSIS/启动文件/链接脚本` 平台证据链 | 第6节到第8.4节 | 工程结构细节继续到第02章，启动与内存布局继续到第03章 |
| Debug 构建产物、`.map/.list` 和容量边界 | 第8.5节到第8.6节、第10节 | 构建产物不能替代芯片丝印、板卡照片、下载日志或运行现场 |
| 全书阅读路径与唯一知识来源 | 本节后续两张全书级表格 | 后续章节只做引用和回顾，避免重复完整讲解平台前提 |

全书阅读路径：

| 读者类型 | 建议阅读方式 | 优先章节 |
|---|---|---|
| STM32 初学者 | 先读各章【必须掌握】，建立平台、时钟、GPIO、调试输出、I2C/USB 和控制主线 | 第01章到第16章，第26章，第31章 |
| 嵌入式开发者 | 在【必须掌握】基础上继续读【工程深化】，重点看构建产物、运行门控、定时器、通信和调试证据 | 第03章到第16章，第26章到第33章 |
| 算法读者 | 先把平台与数据流读通，再集中阅读姿态、滤波、标定、PID 和电机输出链路 | 第17章到第30章，第34章 |
| 维护人员 | 优先阅读【证据与验证】、调试方法、待验证分类和硬件诊断章节，先分清仓库证据与实测证据 | 第04章、第09章到第16章，第32章到第34章 |

核心知识点的完整讲解位置应尽量保持唯一。后续章节若只需要回顾或调用结论，应优先指向下表中的主讲章节，而不是重复完整展开：

| 知识点 | 主讲章节 | 其它章节使用方式 |
|---|---|---|
| 系统时钟、APB 定时器时钟、PSC/ARR 推导 | 第06章、第12章 | 只引用频率推导结果或说明本章使用的配置 |
| GPIO 复用、MSP 初始化和引脚证据 | 第07章 | 只引用引脚模式、复用关系和待验证边界 |
| I2C 主机通信与 MPU6050 读写入口 | 第14章、第18章、第19章 | 传感器章节只引用初始化、连续读取和错误边界 |
| USB FS、CDC 描述符与调试输出链路 | 第15章、第16章 | 后续章节只引用接口边界或日志通道，不重复描述符结构 |
| 姿态表示、角度归一化和 Mahony 主线 | 第23章 | PID、电角和调参章节只引用角度边界与输入含义 |
| 500Hz 控制循环和运行门控 | 第26章、第32章 | 控制章节只引用调用频率、门控和异常防护边界 |
| PID 核心、输出约束和调参路线 | 第27章、第28章、第34章 | 电机输出章节只引用受限后的控制量，不重复 PID 推导 |
| 电角、三相 PWM 与硬件诊断 | 第29章、第30章、第33章 | 调试章节只引用映射、CCR 写入和硬件实测边界 |
...
本节按“现象 -> 可能原因 -> 定位方法 -> 验证步骤 -> 解决方案 -> 经验总结”组织。第01章不调功能现象，而是调平台一致性：先确认工程声明、构建配置、链接边界和实物证据是否处在同一条证据链上。
...
平台章节的调试结论应停在证据能覆盖的层级：工程文件和构建产物能证明当前链接边界，实物板型号、容量读回和运行状态仍需要下载调试或仓库外记录支撑。
...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | `Debug/objects.list` 和 `Debug/makefile` 可以证明 Debug 构建使用了对应启动对象和链接脚本。 | 构建产物不能证明目标板已经运行该 ELF，也不能证明任何外设功能已经进入业务主线。 |
| 软件验证 | `.ioc`、`.cproject`、`.mxproject`、启动文件、链接脚本和 CMSIS 设备头分派共同指向 `STM32F103RCTx` 工程平台。 | 外设启用、初始化顺序、调用路径和业务主线仍需后续章节逐项用源码与构建产物证明。 |
| 参数验证 | `STM32F103xE` 宏、`STM32F103RCTX_FLASH.ld`、LQFP64 和 `Mcu.PinsNb=29` 能给出设备头、RAM/FLASH 边界、封装和引脚分配口径。 | 实际 FLASH/RAM 读数、芯片容量差异、外部晶振参数和板级引脚连线仍需实物或运行记录确认。 |
| 硬件验证 | 仓库内证据可以证明工程面向 `STM32F103RCTx`，并给出封装、引脚分配和内存布局的配置依据。 | 目标板是否实际焊接该芯片、芯片丝印、供电质量、调试连线、外设接线和下载调试是否成功仍需现场证据。 |
| 官方资料待确认 | CMSIS 设备头、启动文件、链接脚本和 STM32F103RCTx/LQFP64 口径可作为本章资料边界。 | 若后续发现目标芯片、封装、容量等级或链接脚本发生变化，需要结合 ST 官方资料重新确认平台边界。 |
| 实验待完成 | 本章已经把工程平台、构建入口、链接边界和仓库外硬件证据拆成可检查对象。 | 后续需记录目标板芯片丝印、下载运行日志、调试连接记录、实际 FLASH/RAM 读数、外部晶振和板级连线证据。 |
