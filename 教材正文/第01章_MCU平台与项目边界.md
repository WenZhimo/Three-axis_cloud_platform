...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 工程平台边界 | `.ioc`、`.cproject`、`.mxproject`、启动文件、链接脚本和 Debug 构建产物共同指向 `STM32F103RCTx` 工程平台 | 本章不判断任何外设功能是否已经进入业务主线，外设启用和调用路径留给后续章节逐项证明 |
| 目标板实物边界 | 仓库内证据可以证明工程面向 `STM32F103RCTx`，并给出 LQFP64、RAM/FLASH 链接边界和 CMSIS 设备头分派 | 目标板是否实际焊接该芯片、是否已经运行该 ELF、下载调试是否成功仍需第33章或现场记录验证 |
| 仓库外硬件边界 | 本章列出了芯片、封装、引脚分配和内存布局的仓库内证据链 | 芯片丝印、实际 FLASH/RAM 读数、外部晶振、供电质量、调试连线和外设接线仍属于仓库外证据 |
