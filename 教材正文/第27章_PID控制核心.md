...
| 阅读层次 | 建议范围 | 适合读者 |
|---|---|---|
| 【必须掌握】 | 第1节到第5节，第13节总结 | 需要理解闭环控制主线、PID 输入输出和三轴调用入口的读者 |
| 【工程深化】 | 第6节到第8.19节，第9节调试方法 | 需要维护 PID 源码、区分三轴路径和定位运行状态的读者 |
| 【拓展阅读】 | 第8.12节到第8.18节，第12节思考题 | 需要进一步理解位置式 PID、D 项状态、`windupGuard` 和后级边界的读者 |
| 【证据与验证】 | 第8节、第9节、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查 PID初始化、参数来源、时间步长保护、角度包裹、P/I/D分项、三轴调用入口、状态写回、`.map/.list/.su/.cyclo` 证据或准备硬件验证的读者 |

如果只是学习 PID 主线，可以先读到第5节并配合第13节总结；调试或审校时再回到第8节各边界小节和第9节逐项查证。

本章速查：

| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| PID 输入、输出和参数结构 | 第4.1节到第4.6节、第5节、第8.1节到第8.6.1节 | 500Hz 调度回到第26章，传感器状态结构回到第25章 |
| 角度误差、时间步长和 D 项状态 | 第4.4节、第4.8节到第4.10节、第8.3节到第8.4节、第8.12节、第8.17节到第8.18节 | D 项滤波和输出约束细节继续到第28章 |
| Pitch、Yaw、Roll 三轴 PID 调用入口 | 第4.5节、第7节、第8.8节到第8.11节、第8.15节 | 三轴系统架构和门控层级继续到第31章 |
| 构建证据、状态写回和调试边界 | 第8.19节到第8.20节、第9节、章节尾部固定检查 | 电角输出链路继续到第29章，三相 PWM 输出继续到第30章 |
...
`updatePID()` 在计算 D 项前先检查：

```text
if lastDcalcValue == 0.0f:
    if D_ERROR: lastDcalcValue = error
    else:       lastDcalcValue = state
```

这段代码的意图是避免第一次进入 PID 时用“尚未建立的上一帧值”计算一个巨大微分尖峰。
但它同时把 `0.0f` 当成两种含义：既表示“D 项历史还没有初始化”，也可能表示“上一帧真实误差或真实状态刚好等于 0”。
因此它不是一个独立的初始化有效标志。

在 `D_ERROR` 模式下，如果当前第一次调用时 `error = 0`，函数会把 `lastDcalcValue` 写成 0，
随后又用 `error - lastDcalcValue` 得到 0，D 项为 0。下一帧如果误差仍然为 0，
同一段初始化判断还会再次成立，但结果仍然是 0；如果下一帧误差突然变为非零，
函数会先把 `lastDcalcValue` 改成当前误差，再计算差分，因此这一帧的原始 D 项仍然可能被压成 0。
也就是说，`lastDcalcValue == 0.0f` 会让“从零误差起步后的第一帧非零误差变化”被当作建立历史值，
而不是当作一次完整微分变化来输出。

这不是运行时错误结论，而是当前源码的状态机边界。它能降低启动瞬态尖峰，但会牺牲一部分初始 D 项响应。
若调参时观察到 D 项第一拍没有按预期响应，应同时记录 `lastDcalcValue`、`error/state`、`dErrorCalc`
和 `zeroPIDstates()` 是否刚刚被调用；缺少逐帧日志时，不能把第一拍 D 项现象直接归因于 `D` 参数本身【待验证】。
...
第8.15至8.20节集中说明 PID 运行状态的边界：哪些字段会跨帧保存，哪些字段只是配置名，哪些证据只能证明路径进入构建。阅读时可先用下面的速查表定位问题类型，再进入对应小节细看：

| PID状态边界 | 优先查看小节 |
|---|---|
| 积分暂停是否只影响当前轴 | 8.15 |
| `windupGuard` 与真实积分限幅的关系 | 8.16 |
| `lastDcalcValue == 0.0f` 的哨兵含义 | 8.17、8.17.1 |
| D 项限幅、滤波和有限值防护 | 8.18 |
| `PIDdata_t` 字段跨帧写回证据 | 8.19 |
| `.map/.list/.su/.cyclo` 能证明的状态路径 | 8.20 |
...
`lastDcalcValue` 变成 `0.0f` 至少有三类来源，调试时不能混写：

| 来源 | 源码证据 | 含义 | 调试边界 |
|---|---|---|---|
| 初始化清零 | `initPID()` 把每个轴的 `lastDcalcValue/lastDterm/lastLastDterm` 写为 `0.0f` | 开机后 D 项历史被主动重置 | 下一次 `updatePID()` 会把 0 当作“需要初始化历史”的哨兵 |
| 门控清零 | `zeroPIDstates()` 也会清空三轴 D 项历史，`.list` 中可见启动门控路径调用它 | 控制接通前可再次丢弃旧 D 项历史 | 不能把清零后的第一帧 D 项当成连续运行下的真实微分响应 |
| 真实历史值为 0 | `updatePID()` 每帧把 `lastDcalcValue` 写成当前 `error` 或 `state` | 上一帧误差/状态可能确实等于 0 | 当前结构体没有独立 valid 标志，无法仅凭字段值区分“真实 0”和“未初始化哨兵” |

因此，观察到 `lastDcalcValue == 0.0f` 时，正确问题不是“D 项是否正常”，而是先问“这个 0 来自清零、来自第一次写入，还是来自上一帧真实值”。`.map` 能证明 `updatePID()`、`zeroPIDstates()` 进入镜像，`.list` 能证明清零和哨兵判断路径存在；但是否在某一帧真的触发了哨兵分支，需要连续断点、日志或诊断标志记录【待验证】。
...
因此，本节只把 `.map/.list/.su/.cyclo` 作为构建层证据：它们能证明 PID 初始化、状态清零、500Hz 调用、三轴 `updatePID()` 路径、角度包裹 helper、后级输出状态对象、静态栈报告和圈复杂度报告进入当前 Debug 构建。
它们不能证明 PID 参数合理、运行时频率达标、`holdIntegrators` 在真实运行中一定触发、传感器方向正确、电机方向安全、机械耦合正确、D 项调参效果符合预期或闭环稳定。
上述结论仍需要断点、日志、波形、硬件响应或仓库外实测记录支撑，缺少证据时保持【待验证】。

## 9. 调试方法

本节按统一调试结构组织：现象 -> 可能原因 -> 定位方法 -> 验证步骤 -> 解决方案 -> 经验总结。对第27章而言，核心是把 PID 初始化、运行参数来源、输入姿态与目标角、时间步长、角度包裹、P/I/D 贡献、积分暂停和 D 项防护分开验证。

### 9.1 现象与可能原因

常见现象包括：PID 历史状态未清零、运行参数和 `config.c` 默认值不一致、某轴没有进入 PID 分支、`dt500Hz` 异常导致 I/D 项突变、角度跨越 `±PI` 时输出跳变、Pitch/Yaw 积分被意外暂停、`windupGuard` 配置值与真实积分限幅不一致，或 D 项第一帧/异常值被防护置零。可能原因包括 `main.c` 启动覆盖参数、Roll 回中分支阻断 PID 调用、控制层和 PID 内部各有角度包裹入口、`holdIntegrators` 是跨轴共享状态，以及源码中积分限幅实际使用硬编码边界。

### 9.2 定位方法：初始化、参数、输入与时间步长
