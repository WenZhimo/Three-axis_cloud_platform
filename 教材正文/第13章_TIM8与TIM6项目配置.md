...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 配置存在边界 | `.ioc`、`tim.c`、启动文件和 CMSIS 能证明 TIM8/TIM6 存在配置、初始化函数、中断入口和部分访问证据 | 不证明它们已经承担当前主控制周期、主 PWM 输出或主时间基准 |
| 业务主线边界 | 源码搜索和 `.list` 能证明当前未发现 `HAL_TIM_PWM_Start(&htim8, ...)`、TIM6 强中断处理函数、TIM6 启动或读取路径 | 外设是否真正进入业务主线，仍需继续查启动调用、回调函数、状态变量变化、寄存器状态和波形实测 |
| 配置推导边界 | TIM8 的 18kHz、TIM6 的 0.5us 计数粒度和 32.768ms 溢出周期可由当前配置推导 | 这些推导不属于实测运行结论，不能替代定时器启动状态、PWM 波形或计数器运行证据 |
| 时钟依赖边界 | 本章能说明 PCLK 与 TIMxCLK 不是同一个概念，且当前频率计算依赖 `SystemClock_Config()` 和 STM32F1 定时器时钟规则 | 若时钟树、APB 分频或定时器时钟规则改变，TIM8/TIM6 频率和周期结论必须重新计算 |
