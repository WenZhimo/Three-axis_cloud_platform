...
| 【证据与验证】 | 第8.6节、第9节、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查 `.map/.list/.su/.cyclo`、`CR1.CEN`、`DIER.UIE`、`BDTR.MOE`、断点和波形证据的读者 |
...
常见现象包括：看到 TIM8 PWM 配置就误以为它是主 PWM 输出，看到 TIM8/TIM6 中断入口就误以为它们提供控制周期，看到 `__HAL_TIM_SET_COUNTER(&htim6, 0)` 就误以为 TIM6 正在计时。可能原因是把 CubeMX 配置、初始化函数、NVIC 入口、启动调用、用户回调和运行寄存器状态混在一起判断。
...
- `.map` 中出现 HAL 启动函数、弱回调或 IRQ 符号时，要先区分丢弃段和最终地址；是否存在项目调用还要结合源码搜索、`.list` 调用点和运行时寄存器状态判断。
- 当前 Debug `.su/.cyclo` 文件显示：`MX_TIM6_Init` 为 16 字节静态栈、圈复杂度 3；`MX_TIM8_Init` 为 96 字节静态栈、圈复杂度 9；相关 MSP 函数也有静态资源记录，但这些记录不证明定时器已经进入运行态。

本章边界：

- 本章证明 TIM8/TIM6 存在配置和部分初始化证据，不证明它们承担当前主控制周期或主 PWM 输出。
- 外设是否真正进入业务主线，必须继续查启动调用、回调函数、状态变量变化和寄存器读写路径。
- TIM8 的 18kHz、TIM6 的 0.5us 计数粒度和 32.768ms 溢出周期均属于配置推导，不属于实测运行结论。
- PCLK 与 TIMxCLK 不是同一个概念；本章的频率计算依赖当前 `SystemClock_Config()` 和 STM32F1 定时器时钟规则，若时钟树改变必须重新计算。
