...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| MCU 侧初始化边界 | 本章证明 GPIO、MSP 和外设复用初始化路径进入当前工程与构建证据 | 不证明外部接线、电平条件、上拉电阻、负载状态或主机连接已经正确 |
| 外设有效性边界 | 本章能定位 I2C、UART、USB、PWM 等外设相关引脚和 MSP/PostInit 配置入口 | 外设是否产生有效通信或输出，还要结合 I2C、UART、USB、PWM 和后续点测章节继续验证 |
