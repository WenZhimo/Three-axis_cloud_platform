...
| 【证据与验证】 | 第8节、第9节、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查全局 MSP 初始化、普通 GPIO 输出、TIM PWM 复用、I2C 开漏、USART3、USB PCD、`.ioc` 与生成代码关系、`.map/.list/.su/.cyclo` 构建产物、运行时 GPIO 寄存器或外部电平证据的读者 |
...
| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| MSP 初始化、GPIO 输出和复用配置主线 | 第4节到第5节、第13节 | 系统时钟回到第06章，AFIO 重映射继续到第08章 |
| `MX_GPIO_Init()`、`HAL_*_MspInit()` 和 PostInit 分工 | 第6节到第8.1节、第9节 | 不把外设参数初始化误读为 GPIO/时钟/NVIC 已全部准备 |
| 普通 GPIO、PWM、I2C、USART3 和 USB PCD 引脚入口 | 第8.2节到第8.7节、第10节 | PWM 回到第12章，I2C/USART/USB 分别回到第14章、第11章、第15章 |
| 构建产物、GPIO 寄存器和外部电气验证边界 | 第8.8节到第8.8.1节、章节尾部固定检查 | `.map/.list` 不能替代 GPIO 运行值、外部电平或 I2C 上拉实测 |
...
还要区分“源码中存在 `HAL_TIM_MspPostInit()` 调用”和“运行时已经执行到 `HAL_TIM_MspPostInit()`”。

以 TIM2 为例，`MX_TIM2_Init()` 会先依次检查这些返回值：

- `HAL_TIM_Base_Init()`
- `HAL_TIM_ConfigClockSource()`
- `HAL_TIM_PWM_Init()`
- `HAL_TIMEx_MasterConfigSynchronization()`
- 多个 `HAL_TIM_PWM_ConfigChannel()`

这些检查都通过后，代码才调用 `HAL_TIM_MspPostInit(&htim2)`。

其中任一步返回非 `HAL_OK`，都会先进入 `Error_Handler()`。本项目 `Error_Handler()` 会执行 `__disable_irq()` 并停在 `while (1)`，因此若失败发生在 PostInit 之前，本轮初始化路径不会继续把 PA1/PA2/PA3 配成 `GPIO_MODE_AF_PP`。
...
#### 8.8.1 `.map/.list` 最终地址与调用路径边界

进一步看当前 `Debug/Three-axis_cloud_platformV2.map`，以下函数都位于 `0x080...` Flash 地址区：

- `MX_GPIO_Init`：`0x080012cc`
- `HAL_I2C_MspInit`：`0x080013d8`
- `HAL_MspInit`：`0x08001a58`
- `HAL_TIM_MspPostInit`：`0x0800230c`
- `HAL_UART_MspInit`：`0x080024bc`
- `HAL_GPIO_Init`：`0x08005b6c`
- `HAL_GPIO_WritePin`：`0x08005e94`
- `HAL_PCD_MspInit`：`0x0800e8d4`

这些 `0x080...` 地址能证明相关函数进入当前最终 Flash 镜像。如果只在 `.map` 的 `0x00000000` 输入段附近看到同名条目，则只能说明对象文件输入段存在，不能单独写成最终镜像可达。

`Debug/Three-axis_cloud_platformV2.list` 提供了更细的调用上下文：

- `HAL_Init()` 调用 `HAL_MspInit()`。
- `main()` 调用 `MX_GPIO_Init()`。
- `MX_TIM2/3/4/8_Init()` 分别调用 `HAL_TIM_MspPostInit()`。
- HAL I2C/PCD/UART 驱动初始化函数内部再调用对应的 `HAL_I2C_MspInit()`、`HAL_PCD_MspInit()` 和 `HAL_UART_MspInit()`。
- `MX_GPIO_Init()`、`HAL_TIM_MspPostInit()`、`HAL_I2C_MspInit()`、`HAL_UART_MspInit()` 内部都能看到对 `HAL_GPIO_Init()` 或 `HAL_GPIO_WritePin()` 的调用位置。
...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | `.map/.list/.su/.cyclo` 能证明 MSP/GPIO 相关入口、HAL 或 `main()` 调用路径、静态栈和圈复杂度条目进入当前 Debug 构建。 | 构建产物不能替代运行时分支命中、GPIO `CRL/CRH` 读回、外部引脚电平、上拉电阻、线缆连接或主机枚举证据。 |
| 软件验证 | 本章能定位全局 MSP、PB12 普通 GPIO、TIM2/TIM3/TIM4/TIM8 PWM 复用、I2C2、USART3 和 USB PCD 的初始化入口。 | 外设是否产生有效通信或输出，还要结合 I2C、UART、USB、PWM 和后续点测章节继续验证。 |
| 参数验证 | `.ioc` 是引脚分配的配置源头，生成代码是 GPIO 模式、速度、复用和端口时钟门控的执行层证据。 | GPIO 端口时钟门控不能直接说明 pin 归属；pin 功能、复用模式、外设实例和时序参数仍需按后续章节逐项确认。 |
| 硬件验证 | 本章能说明 MCU 侧 GPIO、MSP 和外设复用初始化路径进入当前工程与构建证据。 | 不证明外部接线、电平条件、上拉电阻、负载状态、线缆连接、主机连接或外设实物工作状态已经正确。 |
| 官方资料待确认 | HAL MSP、GPIO 复用、开漏/推挽、端口时钟门控和各外设 MSP/PostInit 入口可作为本章资料边界。 | 若后续修改复用关系、GPIO 模式或外设实例，需要结合 STM32F1 参考手册、数据手册和 HAL 实现重新确认。 |
| 实验待完成 | 本章已经把 MCU 侧初始化入口、外设复用入口、构建证据和外部硬件条件拆成可检查对象。 | 后续需记录 GPIO `CRL/CRH` 读回、引脚电平、I2C/UART/USB/PWM 实测、主机连接和点测章节验证结果。 |
