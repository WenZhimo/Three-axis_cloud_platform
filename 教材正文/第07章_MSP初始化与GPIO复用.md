...
| 【证据与验证】 | 第8节到第8.1节、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查构建产物、运行时GPIO寄存器、外部电平、上拉电阻、线缆或主机枚举证据的读者 |
...
- `.map/.list` 能证明 MSP/GPIO 相关入口进入当前镜像并由 HAL 或 `main()` 调用，`.su/.cyclo` 能证明这些函数的静态栈与圈复杂度条目。
