...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 设备栈边界 | 本章证明 USB Device 栈配置、初始化链路和中间件构建证据 | 不证明主机侧枚举已经成功 |
| 枚举状态边界 | 源码和构建产物能定位 USB Device 初始化、PCD 回调桥和 CDC 类注册路径 | `USBD_STATE_CONFIGURED` 需要主机侧枚举过程推进，当前仓库缺少运行证据，标记为【待验证】 |
| 板级硬件边界 | 仓库内能证明 USB FS、PMA、CDC 类和自供电描述符相关配置 | 线缆、供电、D+/D- 板级连接、主机检测、SOF 连续性、PMA 异常、真实吞吐量和自供电声明是否符合板级硬件均需实测 |
| CDC 后续边界 | 第15章只确认 CDC 类初始化、接收函数表和接收函数体进入当前镜像 | CDC 描述符、主动发送接口和业务协议解析需要在第16章继续区分 |
