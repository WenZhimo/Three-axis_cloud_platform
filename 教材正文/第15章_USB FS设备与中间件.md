...
| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| USB 48MHz、PA11/PA12、PCD 和 USBD 初始化主线 | 第4节到第5节、第6.1节到第6.4节、第13节 | 系统时钟回到第06章，GPIO/MSP 和中断入口回到第07章、第09章 |
| `MX_USB_DEVICE_Init()`、类注册、`USBD_Start()` 和回调桥 | 第6.4节到第6.5节、第8.1节到第8.8节、第9节 | CDC 收发接口、描述符和静态内存细节继续到第16章 |
| PMA 端点、EP0、标准请求和设备状态 | 第5节、第6.5节、第8.7节到第8.10.1节、第10节 | 不把 PMA 偏移和 CDC 1024 字节应用缓冲混为一谈 |
| 构建产物、主机枚举、USB 抓包和供电证据边界 | 第8.10节到第8.11节、章节尾部固定检查 | `.map/.list` 不能替代主机侧枚举日志、USB 抓包或线缆/供电实测 |
...
本节按统一调试结构组织：现象 -> 可能原因 -> 定位方法 -> 验证步骤 -> 解决方案 -> 经验总结。对第15章而言，核心是把 USB FS 工程配置、PCD/USBD 初始化链路、设备状态机和主机侧枚举证据分开验证。

### 9.1 现象与可能原因

常见现象包括：主机没有任何 USB 枚举反应、设备停在 `USBD_STATE_DEFAULT` 或 `USBD_STATE_ADDRESSED`、USB 中断不触发、能进入设备栈但主机侧没有 CDC 设备、PMA 溢出或端点处理异常。可能原因包括 USB 48MHz 时钟或 PA11/PA12 配置不一致、PCD/USBD 初始化链路没有完整执行、中断入口没有进入 HAL PCD 处理、描述符或配置阶段没有推进，或缺少主机侧日志/抓包证据。
...
- `objects.list/.map/.list/.su/.cyclo` 的构建产物结论统一回到第10节判断：它们能证明 USB Device Library 核心、CDC 类、USB_DEVICE 适配文件、初始化链路、CDC 类注册、PCD 回调桥、CDC 接收相关函数、静态栈和圈复杂度条目进入某次 Debug 构建；同时 `CDC_Transmit_FS()` 与 `USBD_CDC_TransmitPacket()` 当前处于 `0x00000000` 输入段，不能写成已经存在 CDC 主动发送业务路径，也不能替代 USB 主机侧枚举成功证据。

本章待验证分类：

| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | `objects.list/.map/.list/.su/.cyclo` 能证明 USB Device Library 核心、CDC 类、USB_DEVICE 适配文件、初始化链路、CDC 类注册、PCD 回调桥、CDC 接收相关函数、静态栈和圈复杂度条目进入某次 Debug 构建。 | `CDC_Transmit_FS()` 与 `USBD_CDC_TransmitPacket()` 当前处于 `0x00000000` 输入段，不能写成已经存在 CDC 主动发送业务路径，也不能替代 USB 主机侧枚举成功证据。 |
| 软件验证 | 源码和构建产物能定位 USB Device 初始化、PCD 回调桥、CDC 类注册、reset/setup/data in/out/SOF 回调桥和 CDC 接收函数表。 | `USBD_STATE_CONFIGURED` 需要主机侧枚举过程推进，当前仓库缺少运行证据，仍保持【待验证】。 |
| 参数验证 | 本章已列清 USB FS、PMA 偏移、`USBD_malloc` 静态分配入口、`USBD_SELF_POWERED=1` 和 `low_power_enable = DISABLE` 等配置口径。 | PMA 不是应用层 CDC 缓冲区；自供电声明、PMA 分配、低功耗分支和真实吞吐量仍需结合运行或硬件证据确认。 |
| 硬件验证 | 仓库内能证明 USB FS、PMA、CDC 类和自供电描述符相关配置存在。 | 线缆、供电、D+/D- 板级连接、主机检测、SOF 连续性、PMA 异常、真实吞吐量和自供电声明是否符合板级硬件均需实测。 |
| 官方资料待确认 | USB Device Library、PCD 回调桥、PMA 分配和 CDC 类注册可作为本章中间件结构解释口径。 | 中间件状态机、端点 PMA 语义、自供电声明和低功耗回调仍需结合 ST USB Device/HAL 资料与项目配置确认。 |
| 实验待完成 | 本章已经把设备栈配置、初始化链路、PCD 回调、PMA、CDC 类接收路径和主机枚举边界拆成可检查对象。 | 后续需记录主机枚举、`USBD_STATE_CONFIGURED`、SOF 连续性、端点收发、PMA 异常、供电状态、真实吞吐量和第16章 CDC 主动发送路径。 |
