...
例如 `TIM3->CCR2` 的地址可以按三步还原：

- `stm32f103xe.h` 中 `TIM3_BASE = APB1PERIPH_BASE + 0x00000400`。
- `APB1PERIPH_BASE = 0x40000000`，所以 `TIM3_BASE = 0x40000400`。
- `TIM_TypeDef` 中 `CCR2` 的偏移是 `0x38`，因此 `TIM3->CCR2` 对应的寄存器地址可推为 `0x40000438`。

这不是普通结构体变量寻址，而是对固定外设地址的写入。

同理，Cortex-M3 内核资源也由 CMSIS 头文件给出固定地址：

- `SCS_BASE = 0xE000E000`，`SysTick_BASE = SCS_BASE + 0x0010`，所以 `SysTick` 对应 `0xE000E010`。
- `SCB_BASE = SCS_BASE + 0x0D00`，对应 `0xE000ED00`。
- `DWT_BASE = 0xE0001000`。
- `CoreDebug_BASE = 0xE000EDF0`。

这些地址解释了为什么 CMSIS 名称能直接触达 Cortex-M3 内核资源。
...
当前临界区使用 `__disable_irq()` 后直接 `__enable_irq()`，需要分层理解：

- 当前写法：`drv_pwmMotors.c` 在电机 PWM 更新路径中关闭中断、写定时器寄存器，再重新打开中断。
- 风险边界：如果函数进入时中断本来已经被外层屏蔽，这种写法会在退出时无条件重新打开中断。
- 当前结论：源码中的临界区很短，本章只记录风险边界，不直接判定它已经造成问题。
- 后续改法：若需要提高健壮性，可用 `uint32_t primask = __get_PRIMASK(); __disable_irq(); ... __set_PRIMASK(primask);` 保留进入前状态。
...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | 本章定位了项目直接访问内核寄存器、外设寄存器和 CMSIS 内联函数的位置。 | 访问点是否进入最终镜像、属于主线还是诊断路径，仍需结合后续调用路径、执行上下文和构建产物判断。 |
| 软件验证 | 向量表、`SystemCoreClock`、`SysTick_Config()`、CMSIS 地址映射、`__IO`/`__IOM` 和临界区调用能证明软件结构与访问意图。 | 不逐项推导寄存器完整行为；DWT、SysTick、定时器和 USB 中间件细节留给后续章节。 |
| 参数验证 | `SystemCoreClock / 1000`、`SysTick->LOAD=71999` 和 DWT 微秒等待入口能说明项目中的时间基准参数来源。 | 真实 HCLK、1ms 节拍、DWT 递增频率和微秒等待误差仍需后续章节或实测验证。 |
| 硬件验证 | 源码能证明存在寄存器读写、CMSIS 函数调用和 `CNT/CCR` 等外设访问入口。 | 不能据此证明中断已经发生、PWM 波形正确、外设状态正确或目标板寄存器状态与软件推导完全一致。 |
| 官方资料待确认 | CMSIS 的 `volatile` 语义、内核寄存器命名和中断控制接口为本章访问边界提供基础口径。 | `__disable_irq()` / `__enable_irq()` 不保存进入前中断状态；嵌套场景是否改用 `PRIMASK` 保存恢复仍需结合官方资料和现场路径确认。 |
| 实验待完成 | 本章已经把内核访问、外设寄存器访问、CMSIS 内联函数和临界区边界拆成可追踪对象。 | 后续需验证真实 HCLK、DWT 递增、SysTick 节拍、PWM 波形、外设状态和运行时访问顺序是否满足业务需求。 |
