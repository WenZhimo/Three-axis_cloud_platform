...
| 【证据与验证】 | 第5.1节到第5.3节、第8.5节到第8.10节、第9节、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查 CMSIS 权威定义、构建产物、寄存器读回、DWT计数、SysTick频率、PRIMASK临界区或中断屏蔽状态的读者 |
...
- `__disable_irq()` / `__enable_irq()` 能建立短临界区，但不保存进入前中断状态；嵌套场景要优先考虑 `PRIMASK` 保存恢复。
