...
### 4. `startup_stm32f103rctx.s` 的启动角色
...
### 5. `SystemCoreClock` 与 SysTick 配置
...
- 记录 `SysTick->LOAD`、`SysTick->VAL` 和 `DWT->CYCCNT` 是否符合单调变化预期。
...
验收依据是章节边界表包含 CMSIS 访问证据、USB 中间件保留项和后续章节位置。
...
- `cmsis_gcc.h` 提供 `__disable_irq()` 和 `__enable_irq()` 的编译器内联入口。
...
- `main.c` 使用 `SystemCoreClock / 1000` 配置 SysTick。
...
- 本章说明项目在哪里直接访问内核和外设寄存器，不逐项推导这些寄存器的完整行为。
- 某次寄存器写入属于主线、初始化辅助还是诊断工具，需要结合后续调用路径判断。
- `__IO` 和 `__IOM` 证明寄存器访问具备 `volatile` 语义，不证明多寄存器更新原子、波形正确或外设状态正确。
- 向量表和 `SystemCoreClock` 证明软件结构存在，不证明中断已经发生或真实 HCLK 已经测得。
