...
本项目 TIM2、TIM3、TIM4 的 `AutoReloadPreload` 当前为 Disable，代码也直接写 CCR，因此本章按“直接更新比较值”的路径讲解。
...
它比 DMA 波形输出更简单，也更容易把 PID 输出和三相 CCR 计算放在同一处调试；代价是 CCR 更新时刻由主循环执行位置决定，不是由定时器更新事件严格锁定。
...
当前章节还没有发现项目显式打开输出比较预装载的证据。运行时直接写 CCR 后，新比较值何时影响输出，要以定时器硬件当前计数位置和通道配置为准。

这不是错误，而是一种简单直接的实现。

它的好处是代码路径短；局限是如果在 PWM 周期中途写入 CCR，某些边界情况下可能产生一帧内的瞬态变化。
...
但关闭中断不能把 CCR 更新自动锁定到定时器更新事件，也不能单独证明外部三相波形严格同相同步。相位和边沿一致性仍需要定时器配置、CNT 观察或示波器证据【待验证】。
...
但当前项目的控制更新频率是 500Hz，PWM 载波约 20kHz，并且运行时主要更新 CCR。

在缺少波形实测或异常现象前，教材只能说明“当前未启用 ARR 预装载”，不能直接判定它导致问题。

更细一点说，ARR 预装载和 CCR 预装载不是同一个开关。当前代码明确关闭的是 `AutoReloadPreload`，也就是 ARR 预装载。

是否启用各通道 CCR 预装载，需要继续查看输出比较配置和相关宏调用。本章当前没有发现项目显式开启 TIM2/TIM3/TIM4 输出比较预装载的业务证据。

### 8. TIM8 也配置了 PWM，为什么本章不讲？
