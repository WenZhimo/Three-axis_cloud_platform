...
常见现象包括：某路没有 PWM 输出、频率不是预期的 20kHz、CCR 写入后占空比不变化、三相更新不同步、部分电机轴有输出而另一部分没有输出。可能原因包括 `.ioc`、`tim.c` 和 GPIO 复用不一致，`HAL_TIM_PWM_Start()` 某路没有成功，APB1 定时器时钟与 PCLK1 混淆，`PWM_Motor_SetAngle()` 没有被调用，或外部驱动板、接线和波形测量缺少实测证据。
