...
| 阅读层次 | 建议范围 | 适合读者 |
|---|---|---|
| 【必须掌握】 | 第1节到第5节，第13节总结 | 需要理解 TIM2/TIM3/TIM4 如何用 PSC、ARR、CCR 产生 PWM 输出的读者 |
| 【工程深化】 | 第6节到第8.1节，第9节调试方法 | 需要维护 HAL 初始化、GPIO复用、PWM通道启动和运行时 `TIMx->CCR` 更新路径的读者 |
| 【拓展阅读】 | 第5.1节到第5.4节，第6.1节到第6.2节，第10节到第12节 | 需要进一步理解预装载、边沿对齐、DMA/NVIC边界、载波与控制帧时间尺度的读者 |
| 【证据与验证】 | 第5.1.1节、第6.1节到第6.2节、第8节、第9节、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查 `.ioc/tim.c/drv_pwmMotors.c`、PWM通道、TIM2/TIM3/TIM4初始化、MSP复用引脚、HAL启动状态、运行时CCR写入、构建产物、寄存器状态、DWT/GPIO计时、外部波形或示波器实测边界的读者 |

如果只是沿电机输出基础主线学习，可以先抓住“APB1定时器时钟 -> ARR周期 -> CCR占空比 -> HAL启动通道 -> 运行时直接写CCR”这条链；排查波形、相位或预装载行为时，再回到证据边界和调试方法小节。

本章速查：

| 查找目标 | 优先阅读 | 避免重复展开 |
|---|---|---|
| TIM2/TIM3/TIM4 的 PSC、ARR、CCR 与 PWM 主线 | 第4节到第5节、第13节 | APB1 定时器时钟回到第06章，GPIO 复用回到第07章 |
| HAL 初始化、通道启动和运行时直接写 CCR | 第6节到第8.7节、第9节 | CMSIS 寄存器访问回到第05章，不重复展开 HAL MSP 机制 |
| 预装载、边沿对齐、载波和控制帧时间尺度 | 第5.1节到第6.2节、第10节 | TIM8/TIM6 与多定时器同步继续到第13章、第30章 |
| 构建产物、寄存器状态和外部波形验证边界 | 第8.8节到第8.9节、章节尾部固定检查 | `.map/.list` 不能替代 DWT、GPIO 翻转或示波器实测 |
...
`Debug/Three-axis_cloud_platformV2.map` 可以证明两类构建事实：

- 当前业务路径：`MX_TIM2_Init()`、`MX_TIM3_Init()`、`MX_TIM4_Init()`、`PWM_Motor_Init()`、`PWM_Motor_SetAngle()`、`HAL_TIM_PWM_Init()`、`HAL_TIM_PWM_Start()` 和 `HAL_TIM_PWM_ConfigChannel()` 等符号放入了最终镜像。
- 反向边界：`HAL_TIM_PWM_Start_IT` 和 `HAL_TIM_PWM_Start_DMA` 这类函数存在于 HAL 对象代码中，但仅凭符号出现不能推断项目业务路径已经调用了中断或 DMA 方式更新 PWM。

`Debug/Three-axis_cloud_platformV2.list` 能进一步把调用路径拆成三段：

- 初始化入口：`main()` 依次调用 `MX_TIM3_Init()`、`MX_TIM2_Init()`、`MX_TIM4_Init()`，随后调用 `PWM_Motor_Init()`。
- PWM 启动：`PWM_Motor_Init()` 内部连续调用 9 次 `HAL_TIM_PWM_Start()`。
- 运行时写入：`PWM_Motor_SetAngle()` 内部最终写入 `TIM3->CCR2/CCR3/CCR4`、`TIM3->CCR1`、`TIM2->CCR4/CCR3`、`TIM4->CCR4`、`TIM2->CCR2` 和 `TIM4->CCR3`。

这类证据可以把“源码里写了”推进到“那次构建确实包含这些调用和寄存器写入”。
...
本节按统一调试结构组织：现象 -> 可能原因 -> 定位方法 -> 验证步骤 -> 解决方案 -> 经验总结。对第12章而言，核心是把定时器配置、HAL 启动、运行时 CCR 写入和外部 PWM 波形分开验证。
...
- `.ioc`、`tim.c` 和 GPIO 复用不一致。
- `HAL_TIM_PWM_Start()` 某路没有成功。
- APB1 定时器时钟与 PCLK1 混淆。
- `PWM_Motor_SetAngle()` 没有被调用。
- 外部驱动板、接线和波形测量缺少实测证据。
...
第12章的调试结论应按证据层级收敛：`.ioc/tim.c/MSP` 能证明配置，`PWM_Motor_Init()` 和 `HAL_TIM_PWM_Start()` 能证明启动路径，`PWM_Motor_SetAngle()` 与 `TIMx->CCR` 能证明内部比较值写入，示波器或逻辑分析仪才能证明外部波形真实存在。缺少外部测量时，不把寄存器写入写成电机端三相输出已经验证。
...
- `.map/.list/.su/.cyclo` 的构建产物结论统一回到第8.1节判断：它们能证明 TIM2/3/4 初始化、GPIO 复用、HAL PWM 配置/启动、项目 PWM 启动、运行时 CCR 更新、静态栈和圈复杂度条目进入某次 Debug 构建，但不能替代运行时寄存器、通道启动返回值、真实耗时、20kHz 外部波形或构建新鲜度复核。

本章待验证分类：

| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | `.map/.list/.su/.cyclo` 能证明 TIM2/3/4 初始化、GPIO 复用、HAL PWM 配置/启动、项目 PWM 启动、运行时 CCR 更新、静态栈和圈复杂度条目进入某次 Debug 构建。 | 构建产物不能替代运行时寄存器、通道启动返回值、真实耗时、20kHz 外部波形或构建新鲜度复核。 |
| 软件验证 | `.ioc`、`tim.c` 和 `drv_pwmMotors.c` 能证明 TIM2/TIM3/TIM4 的 PWM 配置、HAL 启动入口、运行时 `TIMx->CCR` 更新路径和“HAL 初始化 + 直接 CCR 更新”的实现方式。 | 当前项目没有使用 TIM2/TIM3/TIM4 的业务 DMA 或中断更新 CCR；预装载、更新事件、DMA 和中断不能写成当前主线功能。 |
| 参数验证 | 本章建立 ARR、PSC、CCR、PWM 模式、GPIO 复用、20kHz 载波约 50us、500Hz 控制帧约 2ms 和每帧约 40 个 PWM 周期的计算口径。 | ARR/PSC/CCR 推导不能证明 9 路 PWM 均启动成功、实际占空比准确或真实控制帧耗时满足预算。 |
| 硬件验证 | 软件路径能说明 `PWM_Motor_SetAngle()` 通过写 `TIMx->CCR` 请求改变占空比。 | 电机相线是否收到有效驱动、外部 20kHz 波形、通道寄存器状态、启动返回值、相位关系和硬件点测仍需实测。 |
| 官方资料待确认 | ST `RM0008`、`AN4013`、`AN4776` 可支撑 PWM1、向上计数、预装载、更新事件和通用定时器能力的背景解释。 | 三相正弦、电角换算、多定时器相位和闭环输出结论应留到后续电机章节，并结合项目源码与硬件记录确认。 |
| 实验待完成 | 本章已经把 PWM 配置、HAL 启动、运行时 CCR 更新、通用 PWM 参数和后续电机链路边界拆成可检查对象。 | 后续需逐路记录启动返回值、运行寄存器、CCR 更新、20kHz 波形、真实执行耗时、系统栈水位和构建新鲜度。 |
