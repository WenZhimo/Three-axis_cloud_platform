...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| PWM配置与启动边界 | `.ioc`、`tim.c` 和 `drv_pwmMotors.c` 能证明 TIM2/TIM3/TIM4 的 PWM 配置、HAL 启动入口和运行时 CCR 更新路径 | 不能据此确认电机相线实际收到有效驱动，9 路通道启动返回值和寄存器状态仍需逐项观测 |
| 后续电机链路边界 | 本章建立 ARR、PSC、CCR、PWM 模式和 GPIO 复用的通用 PWM 基础 | 三相正弦、电角换算、多定时器相位和硬件点测留到后续电机章节继续验证，不能在本章提前写成闭环输出结论 |
| 构建与运行边界 | `.map/.list/.su/.cyclo` 能证明 TIM 初始化、GPIO 复用、HAL PWM 配置/启动、项目 PWM 启动、CCR 更新和静态资源条目进入某次 Debug 构建 | 不能写成 9 路 PWM 均启动成功、系统最大栈水位、真实执行耗时、20kHz 外部波形或构建新鲜度复核证据 |
