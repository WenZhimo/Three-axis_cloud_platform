...
从源码层面可以判断强符号覆盖弱符号；若要取得最终链接层证据，可以在 `.map` 文件中查 `_write` 的提供对象，或在调试器里给 `usart.c` 的 `_write()` 设置断点。没有 map 或断点记录时，运行时路径仍可保持【待验证】。
...
项目使用 115200 和 16 倍过采样；若 APB1 时钟为 36MHz，则理想分频量约为：

```text
USARTDIV = 36,000,000 / 115,200 ≈ 312.5
```
