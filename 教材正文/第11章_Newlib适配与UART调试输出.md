...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 输出主线边界 | 源码和 `.map/.list` 能证明当前 `printf()` 通过强 `_write()` 重定向到 USART3，并调用 `HAL_UART_Transmit()` | 不能证明 USB CDC 已承担调试输出主线，也不能把 UART 软件路径写成主机端已收到完整日志 |
| 外部接收边界 | `.ioc`、`usart.c` 和 HAL 初始化能证明 USART3、PC10/PC11 和阻塞发送路径配置 | 串口输出通路成立仍依赖目标板接线、终端参数、USB 转串口、PC10 波形和阻塞发送耗时的仓库外观察 |
| Newlib/堆运行边界 | `.map` 能证明浮点格式化库成员、`_write_r -> _write` 和 `_sbrk_r -> _sbrk()` 进入当前 ELF | 不能替代断点、堆顶变化、`_sbrk()` 返回值、运行水位、堆峰值或栈水位证据 |
