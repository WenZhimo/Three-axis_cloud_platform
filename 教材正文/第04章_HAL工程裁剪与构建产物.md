...
| 【证据与验证】 | 第8.6节到第8.7节、第9节、章节尾部固定检查，以及所有 `【待验证】` 项 | 需要审查 `.map/.list/.su/.cyclo`、构建日志、下载日志、栈水位和运行现场证据的读者 |
...
`.su` 文件还需要继续拆读。GNU 的[静态栈使用分析](https://gcc.gnu.org/onlinedocs/gnat_ugn/Static-Stack-Usage-Analysis.html)说明把 `.su` 行组织为“源位置/函数、字节数、限定标签”三类信息；本项目的 `.su` 行也符合这个结构，例如 `main` 为 `56 static`，`SystemClock_Config` 为 `96 static`，`computeMotorCommands` 为 `120 static`，`SysTick_Handler` 为 `16 static`。当前 46 个 `.su` 文件中，单函数估算较高的条目包括 `USB_EPStartXfer` 的 `272` 字节、`HAL_UART_IRQHandler` 的 `240` 字节和 `computeMotorCommands` 的 `120` 字节。它们回答的是“单个函数帧估算有多大”，不是“系统任意时刻最大栈深度是多少”。

把 `.su` 变成工程判断时，至少要再加三层证据。第一层是调用链：如果 A 调用 B，再调用 C，不能只取 B 或 C 的单项最大值，而要沿实际调用路径分析当前仍在栈上的帧。第二层是异常和中断：主循环执行到某处时可能被 SysTick、TIM 或 USB 中断打断，ISR 栈帧会叠加在被打断上下文之上。第三层是构建配置：当前 `.su` 来自 `-O0`、`-g3`、`-fstack-usage` 的 Debug 构建，切换优化等级、内联策略或库选项后，估算值可能变化。因此第04章只能把 `.su` 定位为静态入口证据，不能把最大 `.su` 数字直接写成 MSP 最大占用。
...
- 头文件启用、源文件参与构建、对象文件进入链接和业务代码调用是四层不同证据。
- 工程级编译宏、HAL 模块宏、弱符号覆盖和函数运行事实也必须分层判断。
- 本章只确认构建链路和模块可用性，不把库文件存在解释为项目功能已经运行。
- 构建产物存在不等于产物来自最新源码，也不等于已经下载到目标板运行。
- `.su` 静态栈估算可以提示风险入口，但不能替代运行期栈水位或异常现场证据。
