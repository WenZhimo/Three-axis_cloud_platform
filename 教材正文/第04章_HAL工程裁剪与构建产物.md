...
| 类别 | 已由本章证明 | 仍保持【待验证】 |
|---|---|---|
| 构建验证 | 头文件启用、源文件参与构建、对象文件进入链接、业务代码调用、`.map/.list/.su/.cyclo` 产物、18 个 HAL/LL 对象和 46 个 `.su/.cyclo` 条目可以作为分层构建证据。 | 任一单层证据都不能直接写成项目功能已经运行；构建产物存在不等于来自最新源码，也不等于已经下载到目标板运行。 |
| 软件验证 | 工程级编译宏、HAL 模块宏、弱符号覆盖、链接符号和项目强 `HAL_MspInit()` 能说明当前构建选择、模块可用性和可替换入口。 | 不能替代函数运行事实、业务调用路径、运行时分支命中，或外设功能已经被业务主线采用。 |
| 参数验证 | `USE_HAL_DRIVER`、`HAL_*_MODULE_ENABLED`、`nano.specs`、`nosys.specs`、浮点 printf/scanf 选项和 HAL 驱动对象数量已经按当前工程口径列清。 | 这些配置会影响 C 库符号和链接结果，但不能证明运行时内存占用、堆栈水位、外设行为或业务输出正确。 |
| 硬件验证 | 本章确认 HAL 构建链路、模块可用性和静态构建产物存在。 | 不把库文件存在、对象文件进入链接或 HAL 初始化入口存在解释为外设已经在目标板上正确运行。 |
| 官方资料待确认 | HAL 弱符号覆盖、工程级宏、模块级宏、newlib/nosys 规格和 GCC 静态分析产物可作为构建证据解释口径。 | `.su` 最大单函数估算值不等于系统最大栈深度；调用链、中断嵌套、运行水位和异常现场仍需结合工具资料与运行记录确认。 |
| 实验待完成 | Debug 目录中的 `map`、`list`、`.su` 和 `.cyclo` 已被整理为后续静态分析入口。 | 后续需复核构建新鲜度、下载镜像、运行期栈水位、真实调用深度、异常现场、硬件运行证据和业务路径命中。 |
